...
showPreview=function(){
  ...
  const panel=overview+`<details class="card" open><summary>商品露出チェック</summary>${audit.exposure.summary.map(line=>`<p class="tiny">${html(line)}</p>`).join('')}<p class="tiny muted">月間販売予定・販売状態・priority・広告候補・直近10/20実投稿の露出を参照。均等ローテーションは行いません。</p></details>`+researchPanel+`${issues.length?`<div class="notice"><strong>要修正 ${issues.length}件：このまま保存できません。ChatGPTで10日全体を再作成してください。</strong>${issues.slice(0,24).map(note=>`<p>${html(note.text)}</p>`).join('')}${issues.length>24?`<p>ほか${issues.length-24}件。繰り返しの多い企画は期間全体で見直してください。</p>`:''}</div>`:''}`;
  form.querySelector('.editorial-day-preview')?.insertAdjacentHTML('beforebegin',panel);
  const save=form.querySelector('.save-foot button');if(issues.length){save.disabled=true;save.textContent='要修正・保存できません';}
  else if(research.length){save.disabled=true;save.textContent='出典確認後に保存';form.querySelector('#editorialResearchConfirm').onchange=event=>{preview.researchConfirmed=event.target.checked;save.disabled=!event.target.checked;};}
};
